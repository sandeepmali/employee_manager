...
import 'package:employee_manager/employee_details.dart';
...
void main() async {
  runApp(
    BlocProvider(
      create: (context) => EmployeeManagerBloc(),
      child: MyApp(),
    ),
  );
}
...
class _HomPageState extends State<HomPage> {
          ...
          SingleChildScrollView(
            child: Padding(
              ...
              child: Column(children: [
                Row(children: [
                  Flexible(
                      child: customTextField("IP address", ip,
                          onchanged: (p0) => ip = p0,
                          keyboardType: TextInputType.number)),
                  const SizedBox(width: 10),
                  Flexible(
                      child: customTextField("Port",port,
                          onchanged: (p0) => port = p0,
                          keyboardType: TextInputType.number))
                ]),
                customTextField("Database Name",databaseName,
                    onchanged: (p0) => databaseName = p0),
                customTextField("Username", username, onchanged: (p0) => username = p0),
                customTextField("Password", password, onchanged: (p0) => password = p0),
  ...
  TextFormField customTextField(String title,String initialValue,
  ...
  execute(String s, BuildContext context) async {
    try {
      if (s == "Read") {
        ...
        // Convert the List<dynamic> to a List<Item>
        List<ListItemModel> items = parsedJson.map((item) => ListItemModel.fromJson(item)).toList();

        Navigator.of(context).push(MaterialPageRoute(
            builder: ((context) => EmployeeDetailsPage(listItemModel: items))));
        print(
